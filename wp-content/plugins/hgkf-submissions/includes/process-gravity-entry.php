...
function processGravityEntryData($entry, $form)
{
    ...
    if (form_id == 3) {
        ...
        processFreeFields($entry, $form);
        debug_to_console("Processing Free fields completed");
...
function processSettings() {
    ...
    global $wpdb;
    $settings = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}submission_settings  where preset = 1");
...
function processParticipants($entry, $form) {
    ...
    if (!empty($entry[44])) {
        $mainParticipant['Parkeerticket (10,-)'] = $entry[44];
        if ($entry[44] == 'Ja') {
            $dbNumberParkingTickets++;
        }
    }
    ...
    if(is_array($usEntry) || is_object($usEntry)) {
        foreach ($usEntry as $keyOneEntry => $oneEntry) {
            ...
            if($participant['Parkeerticket (10,-)'] == 'Ja') {
                $dbNumberParkingTickets++;
            } 
...
function processPaymentFields($entry)
{
    ...
    $dbParkingTicketFree = 0;
    if ($dbReductionCode && $dbReductionCode != '') {
        global $wpdb;
        $sql = "SELECT * FROM " . $wpdb->prefix . "submission_reduction_codes where code = '" . $dbReductionCode . "'";
        $result = $wpdb->get_results($sql, 'ARRAY_A');

        if (count($result) > 0) {
            $dbTicketPrice = $result[0]['ticket_price'];
            // Check for free parking ticket in the reduction code
            $freeParkingTicket = $result[0]['free_parking_ticket'] == 1 ? true : false;
            $dbParkingTicketFree = $result[0]['free_parking_ticket'];
        }
    }
...
function processFreeFields($entry, $form)
{
...
function saveSubmission()
{
    ...
    debug_to_console('Inserted submission with id: ' . $dbSubmissionId);

    debug_to_console('Insert into table: ' . $wpdb->prefix . 'submission_invoices');
    $wpdb->insert($wpdb->prefix . 'submission_invoices',
        array(
            'submission_id' => $dbSubmissionId,
    ...
    debug_to_console('Inserted invoice with submission id: ' . $dbSubmissionId);
    ...
    foreach ($dbParticipants as $part) {
        $wpdb->insert($wpdb->prefix . 'submission_participants',
            array(
                ...
                'phone' => $part['Telefoon'],
                'parkingticket' => $part['Parkeerticket (10,-)'] == 'Ja' ? 1 : 0
            )
        );
    }
    ...
    debug_to_console('Inserted free fields in table');
}
