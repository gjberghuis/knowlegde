...
function render_edit_participant_page() {
    global $wpdb;

    if (isset($_GET['action']) && $_GET['action'] == 'edit_participant' && isset($_GET['id'])) {
        $participant = $wpdb->get_row("SELECT * FROM {$wpdb->prefix}submission_participants WHERE id = " . $_GET['id']);
        ...
        if ($participant) {
            if (isset($_POST['save_participant'])) {
                ...
                $resultParticipant = $wpdb->update($wpdb->prefix . 'submission_participants',
