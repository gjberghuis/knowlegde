...
function render_edit_special_event_submission_page() {
    ...
    if (isset($_GET['action']) && $_GET['action'] == 'edit_submission' && isset($_GET['id'])) {
         ...
         *
         */
        $sql = "SELECT submission.id, submission.submission_id FROM {$wpdb->prefix}submissions AS submission INNER JOIN {$wpdb->prefix}special_events_invoices AS invoice ON invoice.submission_id = submission.submission_id WHERE id = " . $_GET['id'];
        
        //$sql = "SELECT submission.id, invoice.submission_id, invoice.number, submission.active, submission.submission_type, submission.submission_date, submission.organization, invoice.firstname, invoice.lastname, invoice.adress, invoice.zipcode, invoice.city, invoice.email, invoice.extra_information, submission.parking_tickets, submission.reduction_code, submission.notes FROM {$wpdb->prefix}submissions AS submission INNER JOIN {$wpdb->prefix}special_events_invoices AS invoice ON invoice.submission_id = submission.submission_id WHERE id = " . $_GET['id'];
