<?php
    function render_special_events_settings_page()
    {
        global $wpdb;
        $settings = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}special_events_settings where preset = 1");

        echo '</pre><div class="wrap"><h2>Special events instellingen</h2>';
    ...
    add_action('admin_init', 'saveSpecialEventsSettings');
    ...
    function saveSpecialEventsSettings(){
        if (!empty($_POST) && isset($_POST['submit']))
        {
            ...
            {
                global $wpdb;
                $wpdb->update($wpdb->prefix .'special_events_settings',
                    array(
                ...
                wp_redirect(admin_url()  . "?page=my_special_events_overview");
            }
        }
    }
