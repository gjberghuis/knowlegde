...
/*
 * Template Name: Factuur template special event 2019
 * Version: 1.0
 * Description: Een template voor een factuur van een aanmelding van een formulier die de plugin special-events gebruikt
...
$query = "SELECT * FROM " . $wpdb->prefix . "special_events where submission_id = " . $submission_id;
$submission = $wpdb->get_results($query);

$query = "SELECT * FROM " . $wpdb->prefix . "special_events_invoices where submission_id = " . $submission_id;
$invoice = $wpdb->get_results($query);

$query = "SELECT * FROM " . $wpdb->prefix . "special_events_payment_details where submission_id = " . $submission_id;
$payment_details = $wpdb->get_results($query);

$query = "SELECT * FROM " . $wpdb->prefix . "special_events_participants where submission_id = " . $submission_id;
$participants = $wpdb->get_results($query);

$query = "SELECT * FROM " . $wpdb->prefix . "special_events_settings where event_id = '" . $submission[0]->event_id . "'";
...
<div class="container">
    <div class="header">
        ...
        <div class="naw">
            <ul>
                <li><b><?php echo $submission[0]->organization; ?></b></li>
        ...
        <div class="logo">
            <img src="http://www.hetgrootstekennisfestivalvannederland.nl/site/wp-content/uploads/PDF_EXTENDED_TEMPLATES/images/logo-kennisfestival.png">
        </div>
    </div>
    ...
    <div class="participantsInfo">
        <?php $participantsTitle = (count($participants) > 1 ? 'Deelnemers' : 'Deelnemer'); ?>
        <h1><?php echo $participantsTitle ?></h1>
        <div>
            <table class="people">
                <tr>
                    <th>Naam</th>
